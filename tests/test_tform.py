...
def test_equivalent_inputs_projection(
    vector_df, split_df, split_df_t2, minnie_tform_vx
):
    pts_arr = minnie_tform_vx.apply_project(
        "x", np.vstack(vector_df["pt_position"].values)
    )
    ...
    pts_split_t2 = minnie_tform_vx.apply_dataframe(
        "pt_position_soma", split_df_t2, projection="x"
    )
