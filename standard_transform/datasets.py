...
def _minnie_transforms(tform, pia_point) -> TransformSequence:
    angle_offset = 5
...
def _v1dd_transforms(tform, pia_point) -> TransformSequence:
...
def minnie_transform_vx(voxel_resolution=MINNIE_VOXEL_RESOLUTION) -> TransformSequence:
...
def minnie_transform_nm() -> TransformSequence:
...
def v1dd_transform_vx(voxel_resolution=V1DD_VOXEL_RESOLUTION) -> TransformSequence:
...
def v1dd_transform_nm() -> TransformSequence:
...
def v1dd_streamline_nm() -> Streamline:
...
def v1dd_streamline_vx(voxel_resolution=V1DD_VOXEL_RESOLUTION) -> Streamline:
    "Streamline for v1dd dataset for voxel coordinates"
    with open(V1DD_STREAMLINE_POINT_FILE, "r") as f:
        points = np.array(json.load(f))
    return Streamline(
        points, tform=v1dd_transform_vx(voxel_resolution), transform_points=False
    )


def minnie_streamline_nm() -> Streamline:
...
def minnie_streamline_vx(voxel_resolution=MINNIE_VOXEL_RESOLUTION) -> Streamline:
...
class Dataset(object):
    ...
    def transform_res(self, resolution) -> TransformSequence:
    ...
    def streamline_res(self, resolution) -> Streamline:
